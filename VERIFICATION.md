# v1.0.10 · 2026-09-24 岛内对话与模型目录验证

- 本地模型离线回归 109 项、对话及上下文预算 59 项、软件知识库/前台信息 69 项、自定义目录及设置事务 35 项通过；均使用 .NET Framework 4.6 参考程序集编译。普通 build.ps1 已纳入新增离线套件。
- 已校验缓存中的 Qwen3 0.6B 与固定 llama.cpp 运行时完成 7 项真实模型检查：五分钟生成 countdown:300、连续追问改为三分钟生成 countdown:180、PotPlayer 软件知识库上下文生成 volume/media_toggle。输出见 artifacts/ai-runtime-probe/conversation-probe-result.txt；使用合成上下文，未执行真实媒体操作。1.7B/4B 未实测。
- 两场景岛内对话 UI 共 134 项断言通过，覆盖明确点击才启动、已有任务保护、输入/滑块不拖动窗口、关闭取消、过期响应拦截、局部可读衬底、三档玻璃切换及隐藏停止采样。目录切换和恢复按钮均实际触发，提示准确且模型保持关闭。证据及 12 张图位于 artifacts/conversation-ui-1.0.10。
- Water 图使用固定自有像素夹具验证实际折射，不读取用户桌面；两张发送/停止状态图明确为合成样式状态，不代表真实鼠标事件。主按钮悬停/按下白字对比度分别为 5.87:1、7.55:1。独立视觉复核的两项问题修复后 disposition 为 ship；未在真实教室触屏测试。
- 最终 WPF 安装包、卸载器与免安装包构建成功，内嵌载荷 SHA-256 校验通过。WPF 应用 SHA-256 为 6AEAB45B907764CAED9E78525BA4215F526CB66726CFDD2F5453C198235F54D4。原生版仅版本同步，PE/载荷检查通过，应用 SHA-256 为 26F7BB8473E7961D1905394258941B3D19AF8E29CDF329C9D94D5372D11A3908。
- 基础程序兼容目标不变；可选模型仍要求 Windows 10 2004+ x64 / AVX2，Win7 保留规则模式。此次没有执行真实安装、卸载、关机、自启动或安全设置修改。未在实体 Windows 7、Windows 10 1507/17xx 或学校大屏上验收。模型、运行时和测试夹具均不进入源码/发行包。
# v1.0.9 · 2026-09-24 下载热修复验证

- AI 离线回归 109 项通过（原 80 项加官方 CDN、相对跳转、伪造主机与签名参数边界）；使用 .NET Framework 4.6 参考程序集编译。
- 三个固定模型 URL 的线上 HEAD 均实际跳转至 us.aws.cdn.hf.co。域名清单与 Hugging Face 官方 meta.json 对照，并经独立只读复核。
- 正式 LocalAiTransport 在全新隔离目录直接联网下载：llama.cpp b11146 运行时 18,560,055 字节、Qwen3 0.6B 模型 428,970,080 字节；4 项检查通过，包括无缓存、完整 SHA-256 与 GGUF 魔数。证据为 artifacts/redirect-tests-1.0.9/download-result.txt。此次不启动模型；1.7B/4B 未完整下载。
- 两版构建和安装器内嵌载荷 SHA-256 校验通过；原生版仅版本同步。使用 -SkipTests 打包，未重复运行未修改的旧版行为测试。WPF 应用 SHA-256 为 054E6244B349448DBDF1ADCE2EB5D1065CD1A8FBE3F598EDEF55C800FD650477；原生应用为 bdd56a85c741e851b5ccd6317f328e56b4b2f3aa03241df8bc600291507acf4b。
- 未执行真实安装、卸载、关机、自启动修改或系统保护设置变更；下载缓存不进入源码包和发行包。
