...
bool snapshot=false,multitaskSmoke=false;
...
void App::SyncTouchControls(int id){
 ...
 if(id==1120){int value=30+(int)SendMessageW(Child(id),TBM_GETPOS,0,0);if(classroom)engine->settings.activeDotClassroom=value;else engine->settings.activeDotDesktop=value;SetWindowTextW(Child(1121),(Number(value)+L" px").c_str());PositionIsland();}
 if(id==1130||id==1131){int value=(int)SendMessageW(Child(id),TBM_GETPOS,0,0);if(id==1130)pickedTime.wHour=value;else pickedTime.wMinute=value;SetWindowTextW(Child(1132),DateText(LocalToMs(pickedTime)).c_str());InvalidateRect(main,NULL,FALSE);}
}
void App::TouchTimeLayout(){float x=contentX,y=contentY,w=contentW;Button(361,L"完成选择",x+w-194,y+contentH-62,194,54,true);Button(362,L"前一天",x,y+78,150,54);Button(363,L"后一天",x+w-150,y+78,150,54);HWND label=CreateWindowW(L"STATIC",DateText(LocalToMs(pickedTime)).c_str(),WS_CHILD|WS_VISIBLE|SS_CENTER|SS_CENTERIMAGE,U(x+166),U(y+78),U(w-332),U(54),main,(HMENU)1132,instance,NULL);SendMessageW(label,WM_SETFONT,(WPARAM)font,TRUE);children.push_back(label);Slider(1130,pickedTime.wHour,23,x+82,y+188,w-164,54);Slider(1131,pickedTime.wMinute,59,x+82,y+288,w-164,54);for(int i=0;i<2;i++){Button(364+i*2,L"−",x,y+188+i*100,64,54);Button(365+i*2,L"+",x+w-64,y+188+i*100,64,54);}}
void App::PaintTouchTime(Graphics&g){float x=contentX,y=contentY,w=contentW;Text(g,L"选择日期与时间",x,y,w,48,28,INK,true);Text(g,L"小时  "+Number(pickedTime.wHour),x,y+144,w,36,20,INK,true);Text(g,L"分钟  "+Number(pickedTime.wMinute),x,y+244,w,36,20,INK,true);Text(g,L"拖动滑块，或点击 ＋／−；确认后才会应用。",x,y+365,w,36,16,MUTED);}
std::vector<int> App::Tasks(){std::vector<int> tasks;if(engine->ShutdownVisible())tasks.push_back(3);if(engine->countdownActive)tasks.push_back(1);if(engine->stopwatchRunning||engine->StopwatchMs()>0)tasks.push_back(2);for(size_t i=0;i<pendingNotices.size();i++)tasks.push_back(4+(int)i);return tasks;}
...
void App::Navigate(int p){timePicker=0;if(p!=page||pickedTime.wYear==0){pickedTime=LocalTime(NowMs()+3600000);reminderDraft.clear();reminderDaily=false;}page=p;scroll=0;drafts[0]=L"";drafts[1]=L"";drafts[2]=L"";Layout();InvalidateRect(main,NULL,FALSE);}
void App::Layout(){if(!main)return;RECT available;GetClientRect(main,&available);mainDpi=std::max(0.25f,std::min(dpi,std::min(W(available)/1000.0f,H(available)/(classroom?800.0f:700.0f))));for(HWND h:children)DestroyWindow(h);children.clear();styles.clear();if(font)DeleteObject(font);font=CreateFontW(-U(classroom?20:14),0,0,0,FW_NORMAL,FALSE,FALSE,FALSE,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,CLEARTYPE_QUALITY,DEFAULT_PITCH,L"Microsoft YaHei");RECT r;GetClientRect(main,&r);baseW=W(r)/mainDpi;baseH=H(r)/mainDpi;float bh=classroom?54:42;
 ...
 if(page==4){int delay[]={30,60,120,180};for(int i=0;i<4;i++)Button(270+i,(Number(delay[i])+L" 分钟后").c_str(),x+i*174,y+144,164,bh);Date(1001,x,y+bh+196,200,bh);Date(1002,x+218,y+bh+196,130,bh,true);if(classroom)Button(360,DateText(LocalToMs(pickedTime)).c_str(),x,y+bh+196,370,bh,false,3);Button(280,L"确认预约关机",x+w-210,y+h-92,190,bh,true,4);if(engine->shutdownAt)Button(281,L"取消关机预约",x+w-220,y+34,196,bh,false,9);}
 if(page==5&&settingsTab==2){Button(331,L"返回设置",x,y,138,44);Slider(1120,ActiveDotSize()-30,20,x+20,y+124,w-40,54);Button(356,L"−",x+20,y+194,64,54);Button(357,L"+",x+w-84,y+194,64,54);HWND label=CreateWindowW(L"STATIC",(Number(ActiveDotSize())+L" px").c_str(),WS_CHILD|WS_VISIBLE|SS_CENTER|SS_CENTERIMAGE,U(x+100),U(y+194),U(w-200),U(54),main,(HMENU)1121,instance,NULL);SendMessageW(label,WM_SETFONT,(WPARAM)font,TRUE);children.push_back(label);Button(358,L"恢复场景默认",x,y+h-62,218,54);}
...
void App::Paint(Graphics&g){Quality(g);g.ScaleTransform(mainDpi,mainDpi);g.Clear(PAPER);Box(g,0,0,baseW,78,WHITE,0.1f);Brand(g,26,20,36);Text(g,L"浮岛",74,13,70,48,25,INK,true);SYSTEMTIME now;GetLocalTime(&now);wchar_t clock[60];swprintf(clock,60,L"%d月%d日   %02d:%02d",now.wMonth,now.wDay,now.wHour,now.wMinute);Text(g,clock,baseW-244,17,216,40,16,MUTED,false,2);
 ...
 else if(page==5&&settingsTab==2){Text(g,L"有任务时自动变成计时缩略图",x,y+68,w,40,24,INK,true);Text(g,L"30–50 px · 无任务时恢复小点 · 大屏保留更大的触控区域",x,y+286,w,44,small,MUTED);}
 else if(page==5){
...
void App::Command(int id){try{
 ...
 if(id==356||id==357||id==358){int value=id==358?(classroom?48:36):std::max(30,std::min(50,ActiveDotSize()+(id==356?-1:1)));SendMessageW(Child(1120),TBM_SETPOS,TRUE,value-30);SyncTouchControls(1120);Save();return;}
 ...
 if(id==330||id==331||id==333){settingsTab=id==330?1:id==333?2:0;Layout();return;}
...
void Overlay::Draw(Graphics&g){
 if(kind==3){hits.clear();auto tasks=app->Tasks();bool active=!tasks.empty();int diameter=active?app->ActiveDotSize():app->engine->settings.islandDotSize;bool top=app->engine->settings.dock==Dock::Top,left=app->engine->settings.dock==Dock::Left;int x=top?(width-diameter)/2:(left?2:width-diameter-2),y=top?2:(height-diameter)/2;
  ...
  if(tasks.size()>1){float badge=diameter>=44?14:12;Box(g,x+diameter-badge+1,y-1,badge,badge,INK,badge/2);Digits(g,tasks.size()>99?L"99+":Number((int)tasks.size()),x+diameter-badge+1,y-1,badge,badge,tasks.size()>9?8:10,WHITE);}return;
 }
 ...
 if(kind==0&&!tucked){float size=app->classroom?54:48;float x=(w-size)/2,y=(h-size)/2;if(mode==0)for(int i=6;i>0;i--)Box(g,x-i,y+3-i,size+2*i,size+2*i,Color(3,25,39,79),(size+2*i)/2);brand(x,y,size);}
  ...
  for(int row=0;row<rows;row++){int index=app->taskPage*3+row;if(index>=count&&count)break;int task=count?tasks[index]:0;float y=10+row*126;GraphicsState state=g.Save();float reveal=std::min(1.0f,std::max(0.0f,opening*1.2f-row*.08f));if(!snapshot)g.TranslateTransform(0,(1-reveal)*(1-reveal)*14);
   ...
   else if(task==3)action(L"取消关机",y+36,31,true);else if(task>=4)action(L"知道了",y+36,task*10+1,true);
...
void App::CheckMultitask(){
 ...
 for(int scene=0;scene<2;scene++){
  ...
  Navigate(5);Command(333);SendMessageW(Child(1120),TBM_SETPOS,TRUE,0);SendMessageW(main,WM_HSCROLL,TB_ENDTRACK,(LPARAM)Child(1120));check(ActiveDotSize()==30,"Thumbnail slider lower endpoint");SendMessageW(Child(1120),TBM_SETPOS,TRUE,20);SendMessageW(main,WM_HSCROLL,TB_ENDTRACK,(LPARAM)Child(1120));check(ActiveDotSize()==50,"Thumbnail slider upper endpoint");Command(358);check(ActiveDotSize()==(classroom?48:36),"Thumbnail scene reset");Capture(prefix+L"task-size-settings",main);Command(331);
