...
Model and lightweight rule suggestions are separate opt-in capabilities. WPF can separately download pinned llama.cpp CPU x64 and Qwen3 0.6B / 1.7B / 4B on Windows 10 2004+ with AVX2; no model is bundled. Native Win7 offers clearly labeled rule shortcuts only. The original v1.0.8 context used foreground process basename and selected scene; v1.0.10 extends this as described below. Model proposals are bounded declarative actions; user gestures perform volume/media/countdown/page navigation. Existing timers and urgent notices take priority. Both editions support per-event local WAV/MP3 alert choices, preview, volume and bounded playback. See LOCAL_AI.md for limits and sources.

## Island text conversation and local model directory (v1.0.10)

The WPF radial-menu center, assistant page and tray can open a text conversation in the existing island. This adds no microphone or voice input. Recent conversation remains in application memory, bounded to the latest eight user/assistant messages; it is not a persistent personal knowledge store. Clear starts a fresh conversation. Enter sends, Shift+Enter inserts a line break, and Stop or hiding the conversation cancels the active request. Requests and model context are bounded; proposed volume, media, countdown and page-opening actions still require a user gesture. Existing timing cards remain available, and urgent notices take priority.

Context combines the foreground process, scene and local software knowledge, including PotPlayer executable aliases. Window-title context is a separate opt-in setting, off by default; it does not read page bodies, screenshots or the clipboard. Focusing Free Island preserves the previous external application context. This improves contextual suggestions without claiming unrestricted control of other applications.

The model base directory defaults to `%APPDATA%\FreeIsland\local-ai`; model files live in its `models` child. Users may select a local empty directory or a directory already owned by Free Island, or restore the default. A completed switch stops and disables the model and preserves old files without automatic migration or deletion. Users must explicitly enable an available model or download it in the selected location. Status text states this next step. Optional local inference requires Windows 10 2004 or later, x64 and AVX2; the base application's older-Windows support is unchanged, and Win7 remains rule-only.

Conversation reuses the existing Off / Lite / Water material and bounded refraction, with local readable text backing. No separate glass preference or shipping raster asset is added. The busy indicator is code-drawn, capped at 24 frames per second, respects reduced motion, and stops on cancellation or hiding.
