...
- With no active timing tasks, the collapsed handle remains adjustable from 3 to 20 physical pixels through the 0–100% slider; default 20% is approximately 6 pixels. During timing it becomes a 30–50 physical-pixel thumbnail (default classroom 48, desktop 36, with a custom size setting). It shows a timer value, a thin remaining-progress ring for countdown or a repeating minute indicator for stopwatch, and a numeric task badge when both tasks are present. Clicking expands their separate task cards together, with independent controls and dragging. The original radial-menu launcher retains its role.
