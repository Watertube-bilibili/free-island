...
$setupExecutable = Join-Path $outputRoot 'FreeIsland-Setup-1.0.1.exe'
...
$zipPath = Join-Path $outputRoot 'FreeIsland-Portable-1.0.1.zip'
...
$releaseHashes = @($setupExecutable, $zipPath) | ForEach-Object { (Get-FileHash -LiteralPath $_ -Algorithm SHA256).Hash + '  ' + [System.IO.Path]::GetFileName($_) }
