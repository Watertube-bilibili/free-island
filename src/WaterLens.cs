...
namespace FreeIsland
{
    ...
    internal sealed class WaterLens
    {
        ...
        internal void Shape(double radius, double pressure, double pullX, double pullY, bool compact = false)
        {
            ...
            for (int y = 0, i = 0; y < Height; y++)
            {
                for (int x = 0; x < Width; x++, i++)
                {
                    ...
                    sampleX[i] = (float)(halfW + px / 1.028 - nx * bend);
                    sampleY[i] = (float)(halfH + py / 1.028 - ny * bend);
        ...
        internal void Refract(byte[] backdrop, int backgroundWidth, int backgroundHeight, int backgroundStride,
            double sourceX, double sourceY, double sourceScaleX, double sourceScaleY)
        {
            ...
            if (!Finite(sourceX) || !Finite(sourceY) || !Finite(sourceScaleX) || !Finite(sourceScaleY)) throw new ArgumentException("Non-finite sampling transform.");
