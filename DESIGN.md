...
The ball's edge handles show a small vector arrow over Edge Paper. The island has a separate tiny collapsed handle: a black dot with liquid glass Off, or a transparent water droplet with Lite/Water enabled. Transparent bounds preserve reachability without enlarging the visible mark. Drag release snaps to a supported edge.
...
### Water material and collapsed handle · v1.0.4 · 2026-09-16

The user explicitly requested Apple-inspired clear water on both the expanded island and its collapsed handle. Reuse the existing Off / Lite / Water modes (Win7: Water Motion), keeping Lite as the default and retaining stored preferences. Off draws a black collapsed dot; either glass mode draws a transparent small droplet. Diameter remains 3–20 physical pixels through the 0–100% size slider, default 20% (approximately 6 pixels). Its transparent interaction region remains 44 × 44 DIPs in classroom and 24 × 24 on desktop. Click recovery, dragging, docking and position memory remain available. Do not add a separate material preference or a repeating blink/breathing effect. Expanded scale remains 75–150% on top of the scene, with work-area fitting; ball edge arrows are unchanged.

Modern Windows Water uses bounded in-memory background sampling and convex refraction where supported. Capture exclusion prevents self-feedback and may omit floating windows from compatible recording/sharing, as disclosed in settings. Lite uses clear tint and a thin rim without desktop capture. Win7 supplies clear material and interaction deformation without desktop refraction or Aero blur. Preserve transparent gaps between the six radial targets.

The expanded island's title and detail are slightly larger and gain local translucent backing to aid reading over light, dark or busy backgrounds. Keep that support local to the text so water remains visible around it and modern Windows Water retains edge refraction. Press/drag deforms the material independently of labels. This change does not establish a measured contrast improvement or hardware readability claim. Current release checks belong in VERIFICATION.md; earlier capture reviews above are historical evidence, not v1.0.4 validation.
