...
bool snapshot=false,multitaskSmoke=false,recurringSmoke=false,contextAudioSmoke=false;
...
struct App {
 std::unique_ptr<fi::AlertAudio> alertAudio;
 fiContext::Suggestions suggestions;int systemVolume=50;uint64_t contextPoll=0;bool audioShutdownActive=false;
 void ContextTick();void ContextAction(int id);void ContextVolume(int percent);void AudioCommand(int id);void CheckContextAudio();
...
HWND App::Slider(int id,int value,int maximum,float x,float y,float w,float h){const wchar_t*name=id==1100?L"倒计时小时":id==1101?L"倒计时分钟":id==1102?L"倒计时秒":id==1140?L"提醒音量":id==1120?L"任务球直径":id==1130?L"小时":L"分钟";HWND b=CreateWindowExW(0,TRACKBAR_CLASSW,name,WS_CHILD|WS_VISIBLE|WS_TABSTOP|TBS_HORZ|TBS_NOTICKS|TBS_FIXEDLENGTH,U(x),U(y),U(w),U(h),main,(HMENU)(INT_PTR)id,instance,NULL);SendMessageW(b,TBM_SETRANGE,TRUE,MAKELPARAM(0,maximum));SendMessageW(b,TBM_SETTHUMBLENGTH,U(36),0);SendMessageW(b,TBM_SETPOS,TRUE,value);SetWindowSubclass(b,TouchSliderProc,1,0);children.push_back(b);return b;}
...
std::vector<int> App::Tasks(){std::vector<int> tasks;if(engine->ShutdownVisible())tasks.push_back(3);if(engine->countdownActive)tasks.push_back(1);if(engine->stopwatchActive)tasks.push_back(2);for(size_t i=0;i<pendingNotices.size();i++)tasks.push_back(4+(int)i);if(suggestions.Current()!=fiContext::Category::None)tasks.push_back(-10);return tasks;}
int App::ActiveDotSize(){return classroom?engine->settings.activeDotClassroom:engine->settings.activeDotDesktop;}
void App::TaskAction(int id){if(id>=920&&id<=923){ContextAction(id);return;}if(id==900){CollapseIsland();return;}if(id==901){OpenMain();return;}if(id==902||id==903){int count=(int)Tasks().size();taskPage=std::max(0,std::min(std::max(0,(count-1)/3),taskPage+(id==902?-1:1)));island->Render();return;}if(id>=1000000){primaryTask=id-1000000;handleFrame.clear();return;}if(id>=10000)id-=10000;int kind=id/10,action=id%10;if(kind==1){if(action==1)engine->PauseCountdown();else if(action==2)engine->CancelCountdown();}else if(kind==2){if(action==1)engine->ToggleStopwatch();else if(action==2){engine->ResetStopwatch();laps.clear();}}else if(kind==3){engine->CancelShutdown();alertAudio->StopShutdown();}else if(kind>=4&&(size_t)(kind-4)<pendingNotices.size()){pendingNotices.erase(pendingNotices.begin()+kind-4);alertAudio->Stop();}islandUrgent=engine->ShutdownVisible();PositionIsland();if(island->Visible())island->Render();Layout();islandUntil=GetTickCount64()+7000;}
...
void App::Layout(){if(!main)return;RECT available;GetClientRect(main,&available);mainDpi=std::max(0.25f,std::min(dpi,std::min(W(available)/1000.0f,H(available)/(classroom?800.0f:700.0f))));for(HWND h:children)DestroyWindow(h);children.clear();styles.clear();if(font)DeleteObject(font);font=CreateFontW(-U(classroom?20:14),0,0,0,FW_NORMAL,FALSE,FALSE,FALSE,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,CLEARTYPE_QUALITY,DEFAULT_PITCH,L"Microsoft YaHei");RECT r;GetClientRect(main,&r);baseW=W(r)/mainDpi;baseH=H(r)/mainDpi;float bh=classroom?54:42;
 ...
 if(page==5&&settingsTab==4){
  Button(331,L"返回设置",x,y,138,48);Button(439,L"停止试听",x+w-154,y,154,48);
  for(int i=0;i<3;i++){float row=y+72+i*92;Button(420+i*4,L"选音频",x+w-332,row+25,106,48);Button(421+i*4,L"试听",x+w-216,row+25,98,48);Button(422+i*4,L"默认",x+w-108,row+25,98,48);}
  Slider(1140,alertAudio->VolumePercent(),100,x+116,y+370,w-126,54);
 }
 if(page==5&&settingsTab==5){Button(331,L"返回设置",x,y,138,48);Button(440,engine->settings.contextShortcuts?L"场景快捷：已开启":L"开启场景快捷",x,y+176,270,54,engine->settings.contextShortcuts);Button(441,L"查看标准版下载",x,y+370,250,54);}
 ...
 if(page==5&&settingsTab==0){
  ...
  float foot=(w-40)/5;Button(303,L"找回球",x,y+h-54,foot,48,true,0);Button(304,L"预览岛",x+foot+10,y+h-54,foot,48,false);Button(333,L"任务球",x+2*(foot+10),y+h-54,foot,48,false);Button(335,L"提醒音频",x+3*(foot+10),y+h-54,foot,48,false);Button(336,L"场景快捷",x+4*(foot+10),y+h-54,foot,48,false);
 }
...
void App::Paint(Graphics&g){Quality(g);g.ScaleTransform(mainDpi,mainDpi);g.Clear(PAPER);Box(g,0,0,baseW,78,WHITE,0.1f);Brand(g,26,20,36);Text(g,L"浮岛",74,13,70,48,25,INK,true);SYSTEMTIME now;GetLocalTime(&now);wchar_t clock[60];swprintf(clock,60,L"%d月%d日   %02d:%02d",now.wMonth,now.wDay,now.wHour,now.wMinute);Text(g,clock,baseW-244,17,216,40,16,MUTED,false,2);
 ...
 const wchar_t*title[]={classroom?L"课堂工作台":L"桌面工作台",L"正向计时",L"倒计时",L"日程提醒",L"定时关机",L"设置"};const wchar_t*caption[]={L"设置计时，或查看接下来的日程。",L"开始、暂停或记录分段；需要时切换大屏展示。",classroom?L"拖动滑块设置时长，点开始后才会计时。":L"选择常用时长，或输入小时、分钟和秒。",L"设置一次提醒，或每天在同一时间重复。",L"明确预约时间；关机前会提醒，也可随时取消。",L"调整灵动岛的位置、大小和后台运行方式。"};Text(g,page==5&&settingsTab==4?L"提醒音频":page==5&&settingsTab==5?L"场景快捷":page==5&&settingsTab==1?L"玻璃外观":title[page],contentX,classroom?155:94,contentW-184,46,classroom?28:25,INK,true);Text(g,page==5&&settingsTab==4?L"按提醒类型选择声音，支持本地 WAV 和 MP3。":page==5&&settingsTab==5?L"开启后，在需要时显示建议；由你点击执行。":page==5&&settingsTab==1?L"调整曲面、透明度和高光，即时预览材质。":caption[page],contentX,classroom?200:141,contentW-184,28,classroom?16:13,MUTED);
 ...
 else if(page==5&&settingsTab==4){
  const wchar_t*labels[]={L"倒计时结束",L"日程提醒",L"关机前 10 秒"};
...
void App::Command(int id){try{
 if(id==335||id==336){alertAudio->Stop();settingsTab=id==335?4:5;Layout();return;}
 if(id>=420&&id<=439){AudioCommand(id);return;}
 if(id==440){engine->settings.contextShortcuts=!engine->settings.contextShortcuts;Save();if(!engine->settings.contextShortcuts){suggestions.Observe(L"",GetTickCount64(),false);PositionIsland();if(island->Visible())island->Render();}Layout();return;}
 if(id==441){if(!safe)ShellExecuteW(main,L"open",L"https://github.com/Watertube-bilibili/free-island/releases/latest",NULL,NULL,SW_SHOWNORMAL);return;}
 ...
 if(id==281){engine->CancelShutdown();alertAudio->StopShutdown();islandUrgent=false;Layout();Notify(L"关机计划已停用，之后不会自动关机。");}
 if(id>=290&&id<=292){engine->settings.dock=(Dock)(id-290);Save();PositionIsland();ShowIsland("");Layout();}
 if(id==300){SetStartup(!engine->settings.startup);Save();Layout();}if(id==301){engine->settings.sound=!engine->settings.sound;if(!engine->settings.sound)alertAudio->Stop();Save();Layout();}if(id==302){engine->settings.edgeHide=!engine->settings.edgeHide;Save();RevealBall();SnapBall();Layout();}if(id==303)PositionBall(true);if(id==304)ShowIsland("");
 ...
 if(id==330||id==331||id==333||id==334){alertAudio->Stop();settingsTab=id==330?1:id==333?2:id==334?3:0;Layout();return;}
...
void Overlay::Draw(Graphics&g){
 if(kind==3){hits.clear();auto tasks=app->Tasks();bool active=!tasks.empty();int diameter=active?app->ActiveDotSize():app->engine->settings.islandDotSize;bool top=app->engine->settings.dock==Dock::Top,left=app->engine->settings.dock==Dock::Left;int x=top?(width-diameter)/2:(left?2:width-diameter-2),y=top?2:(height-diameter)/2;
  ...
  if(current==1){ms=app->engine->CountdownMs();fraction=ms/(double)std::max<int64_t>(1,app->engine->CountdownDurationMs());}else if(current==2){ms=app->engine->StopwatchMs();fraction=(ms%60000)/60000.0;}else if(current==3){ms=app->engine->shutdownAt-NowMs();fraction=ms/10000.0;}fraction=std::max(0.0,std::min(1.0,fraction));if(current>=1&&current<=3)g.DrawArc(&progress,rx,ry,ring,ring,-90.0f,(float)std::max(.5,fraction*359.9));
  ...
  if(current>=4||current<0)Icon(g,current<0?7:3,x+diameter*.3f,y+diameter*.3f,diameter*.4f,INK);else{Box(g,x+diameter*.2f,y+diameter*.32f,diameter*.6f,diameter*.36f,Color(215,249,251,254),4);Digits(g,time,(float)x,y+diameter*.17f,(float)diameter,diameter*.66f,time.size()<=2?diameter*.37f:diameter*.25f,INK);}
 ...
 if(kind==0&&!tucked){float size=app->classroom?54:48;float x=(w-size)/2,y=(h-size)/2;if(mode==0)for(int i=6;i>0;i--)Box(g,x-i,y+3-i,size+2*i,size+2*i,Color(3,25,39,79),(size+2*i)/2);brand(x,y,size);}
  ...
  for(int row=0;row<rows;row++){int index=app->taskPage*3+row;if(index>=count&&count)break;int task=count?tasks[index]:0;float y=10+row*126;GraphicsState state=g.Save();float reveal=std::min(1.0f,std::max(0.0f,opening*1.2f-row*.08f));if(!snapshot)g.TranslateTransform(0,(1-reveal)*(1-reveal)*14);
   ...
   else if(task==-10){title=fiContext::Title(app->suggestions.Current());detail=fiContext::Detail(app->suggestions.Current());icon=7;}
   ...
   if(task==-10&&app->suggestions.Current()==fiContext::Category::Media){
    Text(g,L"本地规则 · 系统音量 "+Number(app->systemVolume)+L"%",72,y+42,w-230,26,13,INK);
    float start=78,finish=w-166,cy=y+86;Box(g,start,cy-3,finish-start,6,LINE,3);Box(g,start,cy-3,(finish-start)*app->systemVolume/100.0f,6,BLUE,3);SolidBrush thumb(BLUE);g.FillEllipse(&thumb,start+(finish-start)*app->systemVolume/100.0f-10,cy-10,20.0f,20.0f);volumeTrack=Rect((int)(start*scale),(int)((cy-22)*scale),(int)((finish-start)*scale),(int)(44*scale));hit(start-12,cy-22,finish-start+24,44,924);
   }
   else if(task==1||task==2||task==3)Text(g,detail,72,y+43,w-230,50,28,INK,true);else Text(g,detail,72,y+43,w-230,55,14,INK,false,0,true);
   auto action=[&](const wchar_t*label,float yy,int id,bool primary){Box(g,w-132,yy,110,44,primary?Color(255,232,237,251):Color(235,247,249,253),12);Text(g,label,w-132,yy,110,44,14,INK,false,1);hit(w-132,yy,110,44,(id==920||id==921)?id:10000+id);};
   if(task==1||task==2){action(running?L"暂停":L"继续",y+10,task*10+1,true);action(task==1?L"结束":L"重置",y+62,task*10+2,false);}
   else if(task==-10){action(app->suggestions.Current()==fiContext::Category::Media?L"播放/暂停":L"开始计时",y+10,920,true);action(L"忽略",y+62,921,false);}
   ...
   if(task!=-10)hit(10,y,w-158,116,1000000+task);
   g.Restore(state);
  }
...
void Overlay::Press(POINT p){
 volumeDrag=false;if(kind==2){RECT r=Bounds();POINT local={p.x-r.left,p.y-r.top};for(auto target:hits)if(target.second==924&&Contains(target.first,local)){volumeDrag=true;down=true;app->ContextVolume((int)std::lround(100.0*(local.x-volumeTrack.left)/std::max(1,W(volumeTrack))));return;}}
...
void Overlay::Move(POINT p){
 if(volumeDrag&&down){RECT r=Bounds();app->ContextVolume((int)std::lround(100.0*(p.x-r.left-volumeTrack.left)/std::max(1,W(volumeTrack))));return;}
...
void App::ShowNotice(Notice n){lastNotice=n;if(n.kind=="shutdown"&&n.title==L"即将自动关机"){if(!engine->ShutdownVisible())return;islandUrgent=true;}else{pendingNotices.push_back(n);}islandUntil=GetTickCount64()+20000;PositionIsland();handle->Hide();island->Show();if(engine->settings.sound){if(n.kind=="shutdown"&&n.title==L"即将自动关机"){alertAudio->Play(fi::AlertSoundKind::Shutdown);audioShutdownActive=true;}else if(n.kind=="countdown")alertAudio->Play(fi::AlertSoundKind::Countdown);else if(n.kind=="reminder")alertAudio->Play(fi::AlertSoundKind::Reminder);else if(!safe)MessageBeep(MB_ICONASTERISK);}tray.uFlags=NIF_INFO;wcsncpy(tray.szInfoTitle,n.title.c_str(),63);wcsncpy(tray.szInfo,n.message.c_str(),255);tray.dwInfoFlags=n.urgent?NIIF_WARNING:NIIF_INFO;Shell_NotifyIconW(NIM_MODIFY,&tray);}
...
void App::ContextVolume(int percent){
 if(suggestions.Current()!=fiContext::Category::Media)return;
 percent=std::max(0,std::min(100,percent));
 if(fiContext::SetVolume(percent,safe)){systemVolume=percent;island->Render();}
 else{Notify(L"无法访问系统音量，请检查音频设备是否可用。");}
}
void App::ContextAction(int id){
...
void App::AudioCommand(int id){
 if(id==439){alertAudio->Stop();Notify(L"已停止试听。");return;}
 int row=(id-420)/4,action=(id-420)%4;if(row<0||row>2||action>2)return;
 auto kind=(fi::AlertSoundKind)row;
 if(action==0){
  if(safe){Notify(L"安全预览不打开文件选择器。");return;}
  wchar_t path[32768]={};OPENFILENAMEW dialog={};dialog.lStructSize=sizeof(dialog);dialog.hwndOwner=main;
  dialog.lpstrFilter=L"音频文件 (WAV, MP3)\0*.wav;*.mp3\0\0";dialog.lpstrFile=path;dialog.nMaxFile=32768;dialog.lpstrTitle=L"选择本地提醒音频";dialog.Flags=OFN_FILEMUSTEXIST|OFN_PATHMUSTEXIST|OFN_NOCHANGEDIR;
  if(!GetOpenFileNameW(&dialog))return;
  if(!alertAudio->SetPath(kind,path)){Notify(alertAudio->LastError());return;}
  if(!alertAudio->Save()){Notify(alertAudio->LastError());return;}Layout();Notify(L"音频已保存，点击试听检查效果。");
 }else if(action==1){alertAudio->Play(kind,true);Notify(safe?L"安全预览：不会播放实际声音。":alertAudio->LastError().empty()?L"正在试听，最长 8 秒。":alertAudio->LastError());}
 else{alertAudio->Stop();alertAudio->SetPath(kind,L"");if(!alertAudio->Save()){Notify(alertAudio->LastError());return;}Layout();Notify(L"已恢复系统默认提示音。");}
}
void App::CheckContextAudio(){
 auto check=[](bool ok,const char*message){if(!ok)throw std::runtime_error(message);};check(safe,"Context test requires safe mode");
 engine->CancelShutdown();engine->CancelCountdown();engine->ResetStopwatch();pendingNotices.clear();engine->settings.sound=false;
...
void App::Tick(){
 ...
 if(now-coreTick>=(engine->countdownRunning||engine->stopwatchRunning||engine->ShutdownVisible()?200:1000)){
  ...
  alertAudio->Tick();if(audioShutdownActive&&!engine->ShutdownVisible()){alertAudio->StopShutdown();audioShutdownActive=false;}auto notices=engine->TakeNotices();for(auto n:notices)ShowNotice(n);UpdateTick();ContextTick();if(now>feedbackUntil&&!feedback.empty())feedback.clear();
...
void App::Close(){if(ending)return;ending=true;alertAudio->Stop();try{engine->Save();}catch(...){}KillTimer(main,1);UnregisterHotKey(main,8137);Shell_NotifyIconW(NIM_DELETE,&tray);ShowWindow(main,SW_HIDE);if(stage)DestroyWindow(stage);DestroyWindow(main);main=NULL;PostQuitMessage(0);}
...
void App::RunSmoke(){try{if(contextAudioSmoke){CheckContextAudio();Close();return;}if(recurringSmoke){CheckRecurringShutdown();Close();return;}if(multitaskSmoke){CheckMultitask();Close();return;}
