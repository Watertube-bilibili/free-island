...
if (-not $SkipTests -and (Test-Path -LiteralPath $coreTestSource)) {
    ...
    $conversationTestExecutable = Join-Path $buildDirectory 'LocalAiConversationTests.exe'
    Invoke-Compiler -CompilerArguments (@('/nologo', '/codepage:65001', '/target:exe', '/optimize+', "/out:$conversationTestExecutable") + $referenceArguments + $aiSources + @((Join-Path $projectRoot 'tests\LocalAiConversationTests.cs')))
    ...
    & $updateTestExecutable (Join-Path $projectRoot 'artifacts\update-tests-build-1.0.10')
    ...
    & $recurringTestExecutable (Join-Path $projectRoot 'artifacts\recurring-update-build-1.0.10')
...
$recoveryUninstaller = Join-Path $outputRoot 'FreeIsland-Uninstall-1.0.10.exe'
...
$setupExecutable = Join-Path $outputRoot 'FreeIsland-Setup-1.0.10.exe'
...
$zipPath = Join-Path $outputRoot 'FreeIsland-Portable-1.0.10.zip'
